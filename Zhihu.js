{
	...
	"lastUpdated": "2024-07-10 13:58:49"
}
...
function getIDFromUrl(url) {
	...
	return m
		? { ztype: m[1] === 'p' ? 'article' : 'answer', zid: m[2], url: url }
		: false;
}

function detectWeb(doc, url) {
	const ZID = getIDFromUrl(url);
	// Z.debug(ZID);
	if (ZID) {
...
function getSearchResults(doc, checkOnly, itemInfo) {
	...
	for (let i = 0; i < rows.length; i++) {
		...
		if (!data) { // 搜索页面
			...
			const createTime = rows[i].querySelector("meta[itemprop='dateCreated']");
...
async function scrape(doc, ZID) {
	const { ztype, zid, url } = ZID;
	const newItem = new Zotero.Item(ztype === 'answer' ? 'forumPost' : 'blogPost');
	newItem.url = url;
	if (ztype === 'answer') {
		newItem.title = text(doc, '.QuestionHeader-title');
		...
		newItem.date = ZU.strToISO(text(doc, 'span[data-tooltip]'));
		newItem.notes.push({ note: noteContent });
		newItem.creators.push({ lastName: text(doc, "div.AuthorInfo-head a"), creatorType: 'author', fieldMode: 1 });
		attr(doc, 'meta[itemprop="keywords"]', 'content').split(",").forEach(t => newItem.tags.push({ tag: t }));
	}
	else {
		newItem.title = text(doc, '.Post-Title');
		newItem.abstractNote = attr(doc, 'meta[name="description"]', 'content');
		newItem.blogTitle = text(doc, '.ContentItem-title');
		newItem.websiteType = "知乎专栏文章";
		try {
			const json = await requestJSON(`https://www.zhihu.com/api/v4/articles/${zid}`);
			newItem.date = new Date(json.created * 1000).toISOString();
		}
		catch (error) {
			Z.debug(error);
		}
		newItem.creators.push({ lastName: text(doc, '.AuthorInfo-name'), creatorType: 'author', fieldMode: 1 });
		doc.querySelectorAll('.Tag-content > a').forEach(t => newItem.tags.push({ tag: t.name }));
		// optimal DOM for zhuanlan post
		optimalDOM(doc);
	}
	newItem.language = 'zh-CN';
	const vote = text(doc, '.VoteButton--up').match('[0-9]+$');
